...
func (v *vxlan) ConnectToEndpoint(endpointInfo *natdiscovery.NATEndpointInfo) (string, error) {
	...
	klog.V(log.DEBUG).Infof("================== %v ====================", &remoteEndpoint.Spec.Subnets)
	klog.V(log.DEBUG).Infof("================== %v ====================", &remoteEndpoint.Spec.Subnets)
	klog.V(log.DEBUG).Infof("================== %v ====================", &remoteEndpoint.Spec.Subnets)
	klog.V(log.DEBUG).Infof("================== %v ====================", &remoteEndpoint.Spec.Subnets)
	...
	klog.V(log.DEBUG).Infof("----------------------allowedIPs: %q, remoteVtepIP : %q, v.vxlanIface.vtepIP %q", allowedIPs, remoteVtepIP, v.vxlanIface.vtepIP)
